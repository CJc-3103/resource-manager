...
export default {
  ...
  watch: {
    ...
    // 打开编辑时，初始值会更新，但由于组件没有重新渲染，需要手动获取初始值
    initialVal() {
  ...
  methods: {
    ...
    //   //   let initialVal = this.$attrs.initialVal;
    //   if (initialVal) {
    ...
    resetField() {
      ...
      // 无初始值时需要以下操作
      if ([undefined, null, {}, [], ''].indexOf(initialVal) > -1) {
        switch (this.type) {
          // 单选框无初始值时，默认自动选中第一项
          case 'radio':
            this.val = this.options[0].optKey;
            break;
          // 复选框无初始值时，不选择任何项
          case 'checkbox':
            this.val = [];
        ...
        switch (this.type) {
          case 'checkbox':
            if (!Array.isArray(initialVal))
              initialVal = [].concat([initialVal]);
            break;
          default:
            // 其他情况直接赋值为默认值，因此不需要修改值
            break;
        }
        this.val = initialVal;
      }
